...
void printString_debug(const char *s)
{
#ifdef debug
  while (*s)
    {serial_write(*s);
...
void printString_from_serial2(const char *s)
{serial_write('<<<');
  while (*s)
    {
...
void printInteger_debug(long n)
{
#ifdef debug
  if (n < 0) {
    serial_write('-');
