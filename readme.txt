...
修改增加：$H为多轴复位，$HH为单轴复位
30、20191219_2
修改增加：在软件reset后必须机械homing否则各个轴处于锁定状态无法运动
31、20191220
修改软限位：1不会出现必须reset  
2到位后报警哪个轴到位，然后可以反向运动且角度模式不需要多次倒退，笛卡尔模式还未实现防止多次倒退
32、20191221
1、去掉了逆解函数中对于各轴角度的限位，下方检查到软限位部分！
2、笛卡尔模式下如果触发软限位：如果非插补模式，则可以防止多次倒退，插补模式暂时没做修改
3、更新默认参数中软限位的角度上下限参数
4、修改了硬件限位指令，把grbl原来的（无法使用）改成修改现在的
33、20191221_2
增加了X轴第二个硬件的限位
34、20191221_3
m40校准清零指令，增加自动homing功能
35、20191228
修改了默认的加速度参数，改小到50
修改了B轴最大角度到36
把默认参数中复位归位距离设置为统一的45
